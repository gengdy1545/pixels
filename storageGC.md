...
经代码分析确认以下组件已就绪：
- **RGVisibility/TileVisibility**: 已实现 `getInvalidCount()` / `getTotalRowCount()` 方法（C++ 和 Java/JNI），可获取每个 RG 的精确无效行数用于加权计算
...
*   **指标计算**：在 Memory GC 遍历 Visibility 链表时，统计**无效行数 (Invalid Row Count)**。
    *   **实现位置**: `RGVisibility::getInvalidCount()` / `RGVisibility::getTotalRowCount()` (C++) 和对应的 `RGVisibility.getInvalidCount()` / `RGVisibility.getTotalRowCount()` (Java JNI)
    *   **计算方式**：每个 RG 通过 `getInvalidCount()` 直接返回**无效行数**（baseBitmap 中 1 位的数量之和），通过 `getTotalRowCount()` 返回 RG 总行容量（`tileCount * CAPACITY`）。文件级无效率在 `StorageGarbageCollector` 中精确计算：`fileInvalidRatio = Σ(RG无效行数) / Σ(RG总行容量)`
...
*   **`RGVisibility.h/cpp`**: ✅ 已实现如下方法：
    ...
    *   `uint64_t getTotalRowCount()` - 返回 `tileCount * CAPACITY`（用于精确无效率计算）✅ 本次新增
*   **`RGVisibilityJni.h/cpp`**: ✅ 已实现所有对应 JNI 函数（含本次新增的 `getInvalidCount`/`getTotalRowCount`）
...
*   **`RGVisibility.java`**: ✅ 已实现所有 Storage GC 相关方法：
    ...
    *   `public long getInvalidCount()` / `public long getTotalRowCount()` ✅ 本次新增
    *   保留原有 `public double getInvalidRatio()`（兼容）
...
| `RGVisibility` getInvalidCount/getTotalRowCount (C++) | ✅ **本次新增** | 用于精确加权计算文件无效率 |
| `RGVisibilityJni` 全部函数 | ✅ 已实现 | 含本次新增的 getInvalidCount/getTotalRowCount |
| `RGVisibility.java` 全部公开方法 | ✅ 已实现 | JNI 均为 private native，通过包装方法暴露 |
| `RGVisibility.java` getInvalidCount/getTotalRowCount | ✅ **本次新增** | 含 handle 有效性检查 |
...
- [x] **Phase 7（本次）**: `getInvalidCount()` / `getTotalRowCount()` 新增 + `calculateFileInvalidRatio()` 精确化
