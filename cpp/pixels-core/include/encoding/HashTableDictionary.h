...
public:
    ...
    virtual int add(std::string key) ;
    virtual  int add(const uint8_t* key, int offset, int length);
    virtual  int size();
    virtual void clear();
    virtual void visit(std::shared_ptr<Visitor> visitor);
...
private:
    ...
    void visitKeys(std::shared_ptr<VisitorContext> context, VisitorFunc visitFunc);
};