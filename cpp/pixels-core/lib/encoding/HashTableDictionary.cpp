...
void HashTableDictionary::visit(std::shared_ptr<Visitor> visitor) {
    ...
    visitKeys(context, [visitor](std::shared_ptr<VisitorContext> ctx) {
        visitor->visit(ctx);
    });
}

void HashTableDictionary::visit(VisitorFunc func)
{
    auto context = std::make_shared<VisitorContextImpl>();
    visitKeys(context, func);
}
void HashTableDictionary::visitKeys(std::shared_ptr<VisitorContext> visitor_context, VisitorFunc visitFunc)
{
    auto context = std::dynamic_pointer_cast<VisitorContextImpl>(visitor_context);
    if(context==nullptr) {
        throw std::logic_error("The visitor has not been implemented.");
    }
